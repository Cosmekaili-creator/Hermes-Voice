...
export const POST: RequestHandler = async (event) => {
	...
	const user: Binding = {
		...
		createdAt: now,
		updatedAt: now,
		// Optional persona fields on creation (chunk C) — a fresh user has no existing
		// persona to preserve, so normalizePersona()'s "absent/invalid field falls back to
		// DEFAULT_PERSONA" behavior is exactly right here (unlike the PATCH route, which
		// must use mergePersonaPatch() against the current row instead).
		...normalizePersona(body)
	};
